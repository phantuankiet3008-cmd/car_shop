...
class Product {
    ...
    function list_xe_theo_thuong_hieu($Mathuonghieu) {
        ...
        $sql = "SELECT sp.id_Xe, sp.Ten_Xe, sp.Mo_Ta, sp.Anh_Dai_Dien as Anh_Dai_Dien, sp.Anh_3d, sp.Trang_thai, sp.Ngay_Tao, sp.id_Loai_Xe, sp.id_Thuong_Hieu,
                FROM san_pham_xe sp
                WHERE sp.id_Thuong_Hieu = $Mathuonghieu AND sp.Trang_thai = 'Còn hàng'";
...
// Tìm kiếm sản phẩm: trả về câu SQL và mảng tham số (sử dụng array() để tương thích PHP cũ)
    public function search_san_pham($search = '') {
        $search = trim($search);
        $sql = " SELECT sp.Ten_Xe, lx.Ten_Loai_Xe, th.Ten_Thuong_Hieu
 SELECT  sp.*,
    lx.Ten_Loai_Xe,
    th.Ten_Thuong_Hieu
FROM san_pham_xe sp
...
";
 ...
 function chitietsp($id) {
    ...
    function list_anh_xe_mau($id) {
        $id_xe = (int)$id;
        $sql = "SELECT xma.Hinh_Anh_Xe_Mau as duong_dan 
                FROM xe_mau_anh xma
...
class User {
    ...
    public function laykhachhangtheoid($SDT) {
